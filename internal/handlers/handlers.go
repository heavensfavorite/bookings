...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	A reservation has been made from %s from %s to %s.
...
func (m *Repository) BookRoom(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			// Handle case where no room found, e.g., return user-friendly error or redirect
			// Example:
...
// ShowLogin shows the log in screen
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		// create a map to hold int slices
		...
		for _, y := range restrictions {
			// if it's a reservation, add to reservation map
			if y.ReservationID > 0 {
				// we have a reservation
				for d := y.StartDate; !d.After(y.EndDate); d = d.AddDate(0, 0, 1) {
					reservationMap[d.Format("2006-01-02")] =
						y.ReservationID
				}
			} else {
				// we have a block
...
// AdminPostReservationsCalendar handles the posting of the reservation calendar form
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		curMapInterface := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", x.ID))
		if curMapInterface == nil {
			continue
		}
		curMap, ok := curMapInterface.(map[string]int)
		if !ok {
			log.Printf("block_map_%d in session is not of type map[string]int", x.ID)
			continue
		}

		for name, value := range curMap {
			// Only handle values > 0 which represent blocks
			if value > 0 {
				// If block not in form post, delete it
	...
	for name := range r.PostForm {
		if strings.HasPrefix(name, "add_block") {
			...
			if len(exploded) < 4 {
				log.Println("Invalid add_block format:", name)
				continue
			}
			roomID, err1 := strconv.Atoi(exploded[2])
			t, err2 := time.Parse("2006-01-02", exploded[3])
			if err1 != nil || err2 != nil {
				log.Printf("Error parsing add_block parameters for %s: %v, %v", name, err1, err2)
				continue
			}
